...
export default function CameraCapture({ eventId, userId }) {
  const [isUploading, setIsUploading] = useState({uploading:false,done:0,total:0});
  ...
  const handleFileChange = async (event) => {
    ...
    setIsUploading({uploading:true,done:0,total:files.length});
    ...
    for(let i=0;i<files.length;i++) {
      ...
      setIsUploading({uploading:true,done:i+1,total:files.length});
    }

    setIsUploading({uploading:false,done:0,total})

    
  };

  const {uploading,done,total} = isUploading
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center justify-center gap-4 z-50 bg-theme-2/80 backdrop-blur-md p-3 rounded-full border border-theme-3/20 shadow-2xl">
      ...
      {/* Camera input — no multiple, one shot at a time */}
      <input
        type="file"
        accept="image/*"
        capture="environment"
